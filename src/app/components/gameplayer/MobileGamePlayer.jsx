// src/app/components/GamePlayerMobile.jsx
"use client";
    ...
    <div className="w-full">
      ...
      <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-xs text-muted-foreground">
          {gameActive && loaded && started && (
            <div className="flex items-center gap-2">
              ...
              Game active - Touch outside to scroll
            </div>
          )}
        </div>
        <div className="flex gap-2 justify-center sm:justify-end">
          <button
            ...
            className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1 px-4 py-2 text-sm font-medium
                       rounded-lg transition bg-black/10 hover:bg-black/20 dark:bg-white/10 dark:hover:bg-white/20"
          >
            <span>↻</span> Reload
          </button>
          <button
      ...
      >
        ...
        {status === "checking" && (
          <MobileOverlay coverSrc={coverSrc}>
            <MobileCard title={title} subtitle="Checking game files…" />
          </MobileOverlay>
        )}
        ...
        {status === "ok" && started && !loaded && (
          <MobileOverlay coverSrc={coverSrc}>
            <MobileCard title={title} subtitle="Loading…" />
          </MobileOverlay>
        )}
              ...
              cta={
                <div className="flex flex-col gap-3 w-full">
                  <button
                    ...
                    className="w-full px-4 py-3 rounded-lg bg-white hover:bg-white/90 active:bg-white/80 border border-black/10 font-medium"
                  >
                    ↻ Try again
                  </button>
                  <button
      ...
      {loaded && started && (
        <p className="mt-3 text-sm text-muted-foreground text-center px-4">
          Touch the game to activate controls.  
          Touch outside the game area to scroll the page.
        </p>
      )}
    </div>
  );
...
function MobileCard({ title, subtitle, cta }) {
  return (
    <div className="w-full max-w-sm rounded-2xl bg-white/90 backdrop-blur-md border border-black/10 p-6 shadow-2xl text-center text-slate-900">
      <h2 className="text-2xl sm:text-3xl font-extrabold leading-tight">{title}</h2>
